...
function Header() {
	...
	const handleRegister = () => {
		fetch('http://localhost:3000/users/signup', {
			...
			.then(data => {
				if (data.result) {
					dispatch(login({ username: signUpUsername, token: data.token, canBookmark: data.canBookmark }));
	...
	const handleConnection = () => {
		fetch('http://localhost:3000/users/signin', {
			...
			.then(data => {
				if (data) {
					dispatch(login({ username: signInUsername, token: data.token, canBookmark: data.canBookmark }));
